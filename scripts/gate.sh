...
#   [2/4] rust gate tests   (iso_vic_gate + vic_collision_gate + cart_mapper_gate + expansion_port_gate + u64_turbo_gate + uci_gate, release)
...
printf '[2/4] rust gate tests (iso_vic + vic_collision + cart_mapper + expansion_port + u64_turbo + uci + the daemon suite, release)\n'
command -v cargo >/dev/null 2>&1 || die_red "cargo not on PATH — cannot run gate tests" ""
TLOG=$(mktmp)
if cargo test --release -p trx64-core \
     --test iso_vic_gate --test vic_collision_gate --test cart_mapper_gate --test expansion_port_gate --test u64_turbo_gate --test uci_gate \
     >"$TLOG" 2>&1; then
