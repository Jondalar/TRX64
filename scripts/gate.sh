...
#   [2/4] rust gate tests   (trx64-core unit tests + iso_vic_gate + vic_collision_gate + cart_mapper_gate + expansion_port_gate + u64_turbo_gate + turbo_fastpath_gate + cia_alarm_check_gate + uci_gate + reu_gate + cia_tod_gate + vic_line_trace_gate + ntsc_gate, release)
...
printf '[2/4] rust gate tests (iso_vic + vic_collision + cart_mapper + expansion_port + u64_turbo + turbo_fastpath + cia_alarm_check + uci + reu + cia_tod + vic_line_trace + ntsc + the daemon suite, release)\n'
command -v cargo >/dev/null 2>&1 || die_red "cargo not on PATH — cannot run gate tests" ""
TLOG=$(mktmp)
if cargo test --release -p trx64-core \
     --test iso_vic_gate --test vic_collision_gate --test cart_mapper_gate --test expansion_port_gate --test u64_turbo_gate --test turbo_fastpath_gate --test cia_alarm_check_gate --test uci_gate --test reu_gate --test cia_tod_gate --test sid_multi_gate \
