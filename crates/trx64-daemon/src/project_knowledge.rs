...
/// The active project dir = `bound_project()` ?? cwd. 1:1 with resolve-pc.ts
/// `getProjectDir()` (`process.env["C64RE_PROJECT_DIR"] ?? process.cwd()`); the daemon's
/// run.ts sets `C64RE_PROJECT_DIR` from `--project`, so the `--project` arg layer matches
/// the TS daemon exactly. For callers that RESOLVE paths; a caller deciding whether it may
/// WRITE into a project asks `bound_project()` instead.
