...
fn load_dir_bytes(m: &mut Machine, unit: u8) -> (String, Vec<u8>) {
    ...
    // BASIC prints READY. (`$E1AF`) before it relinks the loaded lines (`$E1B2` → `$A52A`): let the
    // relink finish before reading, or the line links are the drive's dummy `$0101`s.
    frames(m, 5);
