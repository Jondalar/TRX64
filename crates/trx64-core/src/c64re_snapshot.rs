...
pub fn capture_vic_presentation(m: &Machine) -> VicPresentationSnapshot {
    capture_vic_presentation_opts(m, false)
}

/// Spec 807 §4.1 — `capture_vic_presentation` with the framebuffer encode made
/// optional.
///
/// The two framebuffers are 162 240 bytes each and cost **114 µs of the 167 µs** a
/// whole checkpoint capture takes (`perf_bench.rs bench_checkpoint_framebuffer_waste`,
/// release, K=7) — 68 % of the total, and 81.6 % of the rendered tree. Both per-frame
/// producers (the ring's auto-capture and the recorder feed) discard them: they are a
/// DERIVABLE shadow, regenerated by re-sim on scrub/dump (Spec 765 §8, BUG-049 — the
/// anchor is RAM + chip state, not framebuffers).
///
/// Until now that discard happened AFTER the encode, in the daemon
/// (`capture_recorder_anchor_payload`), so the hot path paid for bytes it dropped five
/// lines later. `omit_framebuffer = true` skips the encode instead of undoing it.
///
/// `null` is already a documented value for both fields (see the struct), and
/// `restore_vic_presentation` tolerates it — `ta_u8_decode` returns `None` and the
/// framebuffer is simply not overwritten. Dump/undump/vsf pass `false` and are
/// unaffected.
pub fn capture_vic_presentation_opts(
    m: &Machine,
    omit_framebuffer: bool,
) -> VicPresentationSnapshot {
    VicPresentationSnapshot {
        literal_port_fb: if omit_framebuffer {
            serde_json::Value::Null
        } else {
            ta_u8(m.vic.dbuf.as_ref())
        },
        literal_port_fb_stable: if omit_framebuffer {
            serde_json::Value::Null
        } else {
            ta_u8(m.vic.displayed.as_ref())
        },
...
/// Spec 807 §4.1 — `capture_runtime_checkpoint` with the framebuffer encode made
/// optional. See [`capture_vic_presentation_opts`] for why: with
/// `omit_framebuffer = true` a capture costs ~53 µs instead of 167 µs, because the
/// 114 µs spent encoding two framebuffers the caller then nulls is never spent.
///
/// Callers that PERSIST (snapshot/dump, vsf, undump round-trips) pass `false` and get
/// exactly the tree they always got. The two per-frame producers — the checkpoint
/// ring's auto-capture and the recorder feed — pass `true`.
#[allow(clippy::too_many_arguments)]
pub fn capture_runtime_checkpoint_opts(
    m: &Machine,
    disk_path: &str,
    image_format: &str,
    drive1541: Option<&[u8]>,
...
mod tests {
    ...
    /// Spec 807 G1 — the persist path is untouched. `omit_framebuffer = false` (which
    /// is what `capture_runtime_checkpoint` and every dump/vsf caller pass) must
    /// produce EXACTLY the tree it produced before the flag existed: same fields, same
    /// order, same framebuffer bytes. The flag may only ever subtract, and only from
    /// the two fields it names.
    #[test]
    fn omit_framebuffer_subtracts_exactly_two_fields_and_nothing_else() {
        let mut m = Machine::new();
        m.vic.dbuf[100] = 0x05;
        m.vic.displayed[200] = 0x0b;
        m.ram[0x2000] = 0x42;

        let full = capture_runtime_checkpoint(&m, "/x.d64", "d64", None, None, None, None);
        let full_opt =
            capture_runtime_checkpoint_opts(&m, "/x.d64", "d64", None, None, None, None, false);
        // The default wrapper and an explicit `false` are the same tree.
        assert_eq!(full, full_opt, "omit_framebuffer=false changed the tree");
        // And it really carries the framebuffers.
        assert!(
            !full["vicPresentation"]["literalPortFb"].is_null(),
            "the persist path must still encode literalPortFb"
        );

        let lean =
            capture_runtime_checkpoint_opts(&m, "/x.d64", "d64", None, None, None, None, true);
        // Same top-level shape — no field appeared or vanished.
        assert_eq!(
            full.as_object().unwrap().keys().collect::<Vec<_>>(),
            lean.as_object().unwrap().keys().collect::<Vec<_>>(),
            "omit_framebuffer changed the top-level field set"
        );
        // Every field EXCEPT vicPresentation is byte-identical.
        for (k, v) in full.as_object().unwrap() {
            if k == "vicPresentation" {
                continue;
            }
            assert_eq!(v, &lean[k], "omit_framebuffer altered `{k}`");
        }
        // Inside vicPresentation: exactly the two framebuffers went null, and the
        // three continuation scalars survived unchanged.
        assert!(lean["vicPresentation"]["literalPortFb"].is_null());
        assert!(lean["vicPresentation"]["literalPortFbStable"].is_null());
        for k in ["litLastRasterLine", "lastLitBaLow", "litStableFrameCount"] {
            assert_eq!(
                full["vicPresentation"][k], lean["vicPresentation"][k],
                "omit_framebuffer altered vicPresentation.{k}"
            );
        }
    }

    /// Spec 807 G1b — a framebuffer-less checkpoint still restores the machine, and
    /// the difference against a full one is CONFINED to the framebuffer.
    ///
    /// This is the pair that says what `omit_framebuffer` actually costs a restore.
    /// The full checkpoint brings the framebuffer back; the lean one does not, and
    /// the shadow ends up cleared rather than preserved. That is the documented deal
    /// (Spec 765 §8 / BUG-049: the framebuffer is a derivable shadow, regenerated by
    /// re-sim on scrub) — and it is NOT a change this spec introduced: the daemon's
    /// anchor path nulled both fields after capture long before 807, so every ring
    /// entry ever restored has been a lean one.
    #[test]
    fn lean_and_full_checkpoints_differ_only_in_the_framebuffer() {
        let mut m = Machine::new();
        m.vic.dbuf[100] = 0x05;
        m.vic.dbuf_line = 42;
        m.vic.ba_low_flag = true;
        ...
        // Both restore the MACHINE identically — RAM and the presentation scalars.
        for (label, cp) in [("full", &full), ("lean", &lean)] {
            let mut m2 = Machine::new();
            restore_runtime_checkpoint(&mut m2, cp)
                .unwrap_or_else(|e| panic!("restore {label}: {e}"));
            assert_eq!(m2.ram[0x2000], 0x42, "{label}: RAM must come back");
            assert_eq!(m2.vic.dbuf_line, 42, "{label}: raster line");
            assert!(m2.vic.ba_low_flag, "{label}: ba_low");
            assert_eq!(m2.vic.frame, 7, "{label}: frame count");
        }

        // The one thing that differs: the framebuffer itself.
        let mut m_full = Machine::new();
        restore_runtime_checkpoint(&mut m_full, &full).expect("restore full");
        assert_eq!(
            m_full.vic.dbuf[100], 0x05,
            "the persist path must bring the framebuffer back"
        );

        let mut m_lean = Machine::new();
        restore_runtime_checkpoint(&mut m_lean, &lean).expect("restore lean");
        assert_ne!(
            m_lean.vic.dbuf[100], 0x05,
            "a lean checkpoint cannot carry framebuffer content — if this passes, the \
             flag is not actually omitting anything"
        );
    }
