...
pub struct Session {
    ...
    /// Spec 873/874 — the IEC devices across a power cycle (folders, a host's own): a
...
impl Session {
    ...
    pub fn power_on(&mut self, rom_dir: &std::path::Path) -> Result<(), trx64_core::RomError> {
        ...
        // Spec 873/874 — the IEC devices come back on the bus.
        let devices = std::mem::take(&mut self.iec_devices_held);
        if !devices.is_empty() {
            machine.reattach_iec_devices(devices);
        }
