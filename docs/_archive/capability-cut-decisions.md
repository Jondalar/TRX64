...
**DROPPED 2026-08-11 (the parser half; see the top).** **DECIDED 2026-06-29: Option 2 (= C, refined by the parser-vs-glue split).** The discussion separated two things that were conflated:
...
1. **`mos6502.ts` dedupe** → raw-decode in TRX64. Starts the `trx64-static` crate. Smallest, clearest win (TRX64 monitor already has a decoder).
   **STATUS 2026-07-02: DONE (first slice).** `crates/trx64-static` exists; the
   ...
   `JAM`/`ISC`/`SBC`).
2. ~~**Media format-parse** → `trx64-static`, shared with `vice1541`.~~ **DROPPED 2026-08-11** — no duplication: the drive must refuse what the workbench must read.
3. ~~**Heuristic classifiers** → `trx64-static`, neutral `{offset, kind-guess, confidence}` output.~~ **WON'T-DO 2026-09-19** — TRX64 is a runtime; see the revision at the top.
...
- **TRX64 forever:** emulator (CPU/VIC/SID/drive) · trace/monitor/reverse-debug/snapshot/scrub · render/audio · live media mount · light-local labels · the 6502 decoder its monitor uses (`trx64-static`).

## Still genuinely undecided (smaller, decide at the slice)

- ~~The exact neutral **classification contract** shape (Q1-C) and **media-read API** (Q3-C)~~ — void since 2026-09-19 (Q1 → A) and 2026-08-11 (Q3 dropped).
