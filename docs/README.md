...
| 863 | [NTSC](863-ntsc.md) | **BUILT** | TRX64 half built on `spec-863-ntsc` (§10): `models.toml` with VICE's seven rows (`c64-pal`, `c64-ntsc`, `c64-paln` run; the C64C and old rows are refused by name), the three cycle tables ported with their xpos column, `Machine::timing()` as the one source, NTSC's wrapped window (384×247), the frame-boundary switch (`session/model`, monitor `model`), the model in every snapshot and checkpoint. PAL bit-identical. Closed after the first build: the reverse-debug ring holds its 10 s on every model (sized at the fastest runnable clock), `runtime/scenario_list` names the model a scenario was recorded on, and a switch while recording is journaled at its cycle and replayed there (`runtime/scenario_run` input `kind: "model"`). The C64RE half (§4, §7) is built on C64RE's `spec-863-ntsc`; the two branches merge together. |
---
