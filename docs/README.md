...
| 809 | [Marks and sandboxes](809-marks-and-sandboxes.md) | **PROPOSED** | A named point, and N isolated machines from it. Four monitor verbs (set/list/drop/goto) plus an RPC capability: restore into N instances, put these bytes in, run this long, hand back the end states. **TRX64 has nothing to do with the tests except that it owns the runtime** — it never learns what a branch is, why it exists or whether it won; that is 810. Most of the substrate exists (769.2, 795, 796, 797, 794, 787) and is bound to generated ids rather than names. Adds copy-on-write media folders and multi-line assembly. |
---
