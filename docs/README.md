...
| 851 | [The U64 machine profile, and a CPU that is actually faster](851-the-u64-machine-profile-and-a-faster-cpu.md) | **PROPOSED** | `--machine c64\|u64\|128`, default `c64`, before power-on — `u64` (also `c64u`, `ue2`) is one machine for U64, Elite II and C64 Ultimate, with the speed table as a parameter. It brings the turbo registers from the firmware's own layout (fixes 815's `$D031 = $80` reading as turbo), a real speed-up by a clock divider ported from VICE's TurboMaster so `clk` stays PHI2, and the UCI block of 852. Nothing built. |
| 852 | [The Ultimate Command Interface as U64 hardware](852-the-ultimate-command-interface-as-u64-hardware.md) | **PROPOSED** | `command_protocol.vhd` ported one to one into the `u64` profile — the same block on every Ultimate firmware target. C64 side on 850's port (IRQ, freeze as hold, `$FF00` trigger through the snoop), firmware side as an API a host maps to `CMD_IF_BASE`, and two events the firmware needs (C64 reset, the `$D038`/`$D036` unlock). Disabled without a firmware, as the firmware's own default. Nothing built. |
---
