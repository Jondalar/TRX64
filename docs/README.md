...
| 850 | [The expansion port as a device interface](850-the-expansion-port-as-a-device-interface.md) | **PROPOSED** | UE2's UCI requirements R1–R6, built as a generic interface rather than for UCI: a device beside the cartridge on `$DE00-$DFFF`, side-effect-free peeks, a device stop reason, a banking-independent write snoop (`$FF00`, `$D036`, `$D038` — VICE's own `$FF00` REU hook), one `INT_SRC_EXPANSION` sampled per cycle, and CPU hold as a run state with a reset flavour. Nothing built. |
| 851 | [The U64 machine profile, and a CPU that is actually faster](851-the-u64-machine-profile-and-a-faster-cpu.md) | **PROPOSED** | `--machine c64\|128\|u64` as a start parameter before power-on, the U64 turbo registers from the firmware's own layout (fixes 815's `$D031 = $80` reading as turbo), and a real speed-up by a clock divider ported from VICE's TurboMaster, so `clk` stays PHI2. Nothing built; the name of the flag is the owner's call. |
---
