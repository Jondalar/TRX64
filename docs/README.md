...
| 812 | [Capture scenario + release reel](../../C64ReverseEngineeringMCP/specs/812-capture-scenario-and-reel.md) | **BUILT** | The spec file is C64RE's; the runtime half is here and is done. `trx64cli reel` runs a JSON capture scenario on its OWN machine (a 787 scratch process — no daemon, no port, never the shared session) where every step carries its own duration and every anchor is an absolute cycle, so the same file replays to the same bytes. New: `trx64-core::gif89a` (GIF89a straight from the VIC's 4-bit index buffer against the 16-entry COLODORE table — no quantization, plus the block-structure parser, because scanning for the `21 F9` marker false-positives inside LZW data), `session/frame_indices`, `session/advance_to_frame`. Five gates in `e2e_812_capture_scenario.rs`. The encoder shipped WRONG first and eight of our own tests were green over it — see the spec's §11. |
---
