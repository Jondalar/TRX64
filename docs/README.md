...
| 863 | [NTSC](863-ntsc.md) | **PROPOSED** | A second video standard, chosen before power-on: a video model record (6567R8: 65×263, 1 022 730 Hz, 60 Hz TOD; the cycle table ported 1:1 from VICE `cycle_tab_ntsc`), one timing record every frame and clock consumer reads (today 12 copies of 19656, 7 of the clock), NTSC's wrapped display window, the standard recorded and enforced in every snapshot (a cross-standard restore can panic today). PAL bit-identical. Open: which chips, runtime switch, colour. |
---
