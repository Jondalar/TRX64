...
| 807 | [Binary checkpoint ring](807-binary-checkpoint-ring.md) | **PROPOSED** | JSON + base64 off the per-frame path: capture builds a native struct, `to_value()` renders when someone reads. The `.c64re` format and the WS wire do not change. Prerequisite for a full checkpoint per frame (reverse playback), which today would build 100 JSON trees a second inside the stream loop's lock. |
