...
**Status:** BUILT on branch spec-873-folder (not merged)
...
- **ATN falls:** abort whatever it was doing (talking, listening, idle), release CLK, pull
  ...
  as a request line (Green Beret) — and, answering on the Ultimate's clock rather than a
  1541 DOS's, it changes the KERNAL's schedule where it acknowledges sooner than drive 8
  (Polar Bear). MOTM, which an idle drive 9 breaks, it leaves alone (§14, measured). That
  is the modelled physics, not a defect.
...
7. **The seven games** — no folder: **7/7 PASS, all seven screenshots byte-identical to
   ...
   expected — 4 byte-identical (motm, impossible2, lastninja, maniac), 3 expected to
   differ**, each named in `folder_expected`:
   ...
   - **Polar Bear** — differs in 104 px, two lines of the top border: the folder
     ...
     330 833). The game runs the same on another schedule.
