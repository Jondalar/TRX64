...
> trace-diff-verified 1:1 against the c64re TypeScript headless runtime as its parity
> oracle) — only the language/runtime differs.
...
> **Architecture context.** TRX64 is the strategic runtime base and the default backend;
> c64re is the reverse-engineering workbench, and its TypeScript core is the fallback /
> parity oracle used to trace-diff-verify TRX64. The ~8–10× gap measured here is part of
> the rationale for TRX64 being the default. Leitregel: Capability → TRX64,
